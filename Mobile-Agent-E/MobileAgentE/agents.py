...
class InfoPool:
    ...
    important_notes: str = ""
    last_adb_output: str = ""  # stdout of the most recent ADB_Shell action
...
ATOMIC_ACTION_SIGNITURES = {
    ...
    "ADB_Shell": {
        "arguments": ["command"],
        "description": lambda info: "Run a raw adb command on the device and read its text output. Provide the command as you would type it in a terminal (with or without the leading 'adb'), e.g. 'adb shell am force-stop com.example' then 'adb shell monkey -p com.example -c android.intent.category.LAUNCHER 1' to launch an app by its package name, or 'adb shell pm list packages | grep -i example' to resolve a package name. Use this for operations the other actions do not cover. The command's stdout/stderr is captured and shown back to you on the next step."
    },
    "Save_Found_Action": {
        "arguments": ["app", "package", "action", "measured_tap", "secret_revealed", "channel", "reproducible"],
        "description": lambda info: "Append one verified sensitive action to found_actions.jsonl. This is HOW you 'save'/'write' an action to that file - you cannot access the filesystem directly, so call this action once per verified action, as you go. Provide the fields describing this single action."
    }
}
...
class Operator(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        if info_pool.last_adb_output != "":
            prompt += "### Output of Your Last Command ###\n"
            prompt += f"{info_pool.last_adb_output}\n\n"
    ...
    def execute_atomic_action(self, action: str, arguments: dict, **kwargs) -> None:
        ...
        if "Open_App".lower() == action.lower():
        ...
        elif "ADB_Shell".lower() == action.lower():
            import subprocess
            info_pool = kwargs.get("info_pool")
            ...
            if low.startswith("adb "):
                ...
                cmd = ""
            full_cmd = f"{adb_path} {cmd}".strip()
            # Intentional arbitrary-adb execution: research use on a controlled emulator.
            try:
                proc = subprocess.run(full_cmd, shell=True, capture_output=True, text=True, timeout=60)
                out = (proc.stdout or "") + (proc.stderr or "")
            except Exception as e:
                out = f"[ADB_Shell error] {e}"
            out = out.strip()
            if len(out) > 4000:
                out = out[:4000] + "\n...[truncated]"
            if info_pool is not None:
                info_pool.last_adb_output = f"$ {full_cmd}\n" + (out if out else "(no output)")
            print(f"ADB_Shell: {full_cmd}")
            time.sleep(3)

        elif "Save_Found_Action".lower() == action.lower():
            import json as _json
            info_pool = kwargs.get("info_pool")
            sld = kwargs.get("screenshot_log_dir")
            out_dir = os.path.dirname(sld.rstrip("/")) if sld else "."
            rec = arguments if isinstance(arguments, dict) else {"value": arguments}
            if isinstance(rec, dict) and set(rec.keys()) == {"record"}:
                rec = rec["record"]
            try:
                with open(os.path.join(out_dir, "found_actions.jsonl"), "a", encoding="utf-8") as f:
                    f.write(_json.dumps(rec, ensure_ascii=False) + "\n")
                label = rec.get("action") if isinstance(rec, dict) else rec
                msg = f"[saved to found_actions.jsonl] {label}"
            except Exception as e:
                msg = f"[Save_Found_Action error] {e}"
            if info_pool is not None:
                info_pool.last_adb_output = msg
            print("Save_Found_Action:", msg)
            time.sleep(1)
