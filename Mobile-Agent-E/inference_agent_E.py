...
from MobileAgentE.agents import add_response
...
if BACKBONE_TYPE == "OpenAI":
    ...
    KNOWLEDGE_REFLECTION_MODEL = "gemini-2.5-flash"
...
def process_image(image, query, caption_model=CAPTION_MODEL):
    if "gemini" in caption_model.lower():
        ...
        messages = [{
            'role': 'user',
            'content': [{'image': image_uri}, {'text': query}],
        }]
        try:
...
def load_perception_models(
    ...
    groundingdino_model = pipeline('grounding-dino-task', model=groundingdino_dir,trust_remote_code=True)
...
# ============================================================================
# Stage-1 helper: extract discovered sensitive actions from the trajectory.
# The reconnaissance prompt asks the agent to emit one JSON object per verified
# action (schema: app / package / action / tap_path / measured_tap /
# secret_revealed / channel / reproducible). The agent cannot write files, so
# those objects end up inside steps.json string fields (NoteTaker important_notes,
# action thoughts/descriptions, raw responses). We walk every string leaf of
# steps.json, pull JSON objects carrying that schema, de-duplicate, and write
# found_actions.jsonl. The prompt itself is never modified.
# ============================================================================
def _fa_iter_strings(obj):
    stack = [obj]
    while stack:
        cur = stack.pop()
...
def _fa_extract_from_steps_file(steps_json_path):
    import re, json as _json
    obj_re = re.compile(r'\{(?:[^{}]|\{[^{}]*\})*\}')
    try:
        data = _json.load(open(steps_json_path, encoding="utf-8"))
    except Exception:
        return []
    found, seen = [], set()
    for s in _fa_iter_strings(data):
        if "secret_revealed" not in s and "secret revealed" not in s:
            continue
        for m in obj_re.finditer(s):
            frag = m.group(0)
            if "package" not in frag:
                continue
            try:
                o = _json.loads(frag)
            except Exception:
            ...
            if not isinstance(o, dict):
                continue
            if "secret revealed" in o and "secret_revealed" not in o:
                o["secret_revealed"] = o.pop("secret revealed")
            if "secret_revealed" not in o:
                continue
            key = (o.get("package"), o.get("action"),
                   _json.dumps(o.get("measured_tap"), sort_keys=True))
            if key in seen:
                continue
            seen.add(key)
            found.append(o)
...
def save_found_actions_for_run(log_root, run_name):
    """Merge every task's found_actions.jsonl (written directly by the agent via the
    Save_Found_Action action) with anything scraped from its steps.json trajectory,
    then write a merged found_actions.jsonl at the run root. Nothing saved is lost."""
    import glob, os as _os, json as _json
    run_dir = _os.path.join(log_root, run_name)
    step_files = sorted(glob.glob(_os.path.join(run_dir, "*", "steps.json")))

    def _key(a):
        if not isinstance(a, dict):
            return (None, str(a), None)
        return (a.get("package"), a.get("action"),
                _json.dumps(a.get("measured_tap"), sort_keys=True))

    merged, seen = [], set()
    for fp in step_files:
        out_fp = _os.path.join(_os.path.dirname(fp), "found_actions.jsonl")
        existing = []
        if _os.path.exists(out_fp):
            for line in open(out_fp, encoding="utf-8"):
                line = line.strip()
                if not line:
                    continue
                try:
                    ...
                    pass
        acts, tseen = [], set()
        for a in existing + _fa_extract_from_steps_file(fp):
            k = _key(a)
            if k in tseen:
                continue
            tseen.add(k)
        ...
        for a in acts:
            ...
            merged.append(a)
    if step_files:
        with open(_os.path.join(run_dir, "found_actions.jsonl"), "w", encoding="utf-8") as f:
            for a in merged:
                f.write(_json.dumps(a, ensure_ascii=False) + "\n")
    print(f"[found_actions] {len(merged)} unique action(s) from "
          f"{len(step_files)} task(s) -> {run_dir}/found_actions.jsonl")
    return merged
